...
class PdfGenerator
  ...
  def run(payload)
    ...
    name_w_path = "hosted/#{view_id}.pdf"
  ...
  def generate(url, tmp_file)
    ...
    cmdline = "wkhtmltopdf-amd64 --page-size Letter --margin-top .25in --margin-right .25in --margin-bottom .5in --margin-left .25in --encoding UTF-8 -q #{url} #{tmp_file}"
    puts "#{cmdline}"
    # puts `wkhtmltopdf --page-size Letter --margin-top .5in --margin-right .25in --margin-bottom .5in --margin-left .25in --encoding UTF-8 -q #{url} #{tmp_file}`
    puts `#{cmdline}`
  end
