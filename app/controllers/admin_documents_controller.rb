class AdminDocumentsController < AdminController
  ...
  def edit
    ...
    @users = User.joins(:user_assignments).where(archived: false, user_assignments: { organization_id: @organizations.pluck(:id) }).uniq()
    @users += [@document.user] if !@document.user.blank? && !@users.where(id: @document.user_id)
  end
