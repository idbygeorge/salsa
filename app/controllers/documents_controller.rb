...
class DocumentsController < ApplicationController
  ...
  def course
    ...
    begin
      ...
      if has_role('designer')
        ...
        raise ActionController::RoutingError.new('Not Authorized') and return
      end
    end
    ...
    if @lms_course
      ...
      render :layout => 'edit', :template => '/documents/content' and return
    else
      session[:redirect_course_id] = params[:lms_course_id]

      if params[:document_token]
        redirect_to '/oauth2/login', lms_course_id: params[:lms_course_id], document_token: params[:document_token] and return
      else
        redirect_to '/oauth2/login', lms_course_id: params[:lms_course_id] and return
      end
    end
  ...
  def find_or_create_document session, params, organization, lms_course
    ...
    if params[:document_token] && params[:canvas] && @document
      ...
      redirect_to lms_course_document_path(lms_course_id: params[:lms_course_id]) and return
      ...
      render :layout => 'relink', :template => '/documents/relink' and return
    end
