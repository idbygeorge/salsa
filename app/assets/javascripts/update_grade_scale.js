var updateGradeScale = function(grade_scale, total_points) {
  ...
  if (!$('#gradeUnitsPercent').is(':checked') && $(
      '[data-grades-scale-required-points]').length == 1) {
    requiredPoints = $('[data-grades-scale-required-points]').data(
      'grades-scale-required-points');
  }

  if ((requiredPoints != 0 && requiredPoints != total_points) || (($(
      '#grade_components:visible').length && total_points < 100) || ($(
      '#gradeUnitsPercent').is(':checked') && total_points != 100))) {
  ...
  rows.each(function(index, row) {
    ...
    $(cells[1]).text(lower_points + " - " + upper_points);
